...
function isMeetingLike(content: ScreenpipeContent): boolean {
...
export const ScreenpipeMeetings: React.FC<ScreenpipeMeetingsProps> = ({
  ...
  }, [plugin.settings.screenpipeApiUrl, plugin.settings.screenpipeTimeRange, plugin.settings.queryScreenpipeLimit]);
  ...
  if (unavailable) {
    return (
      <div className={tw("p-4")}>
        <h3 className={tw("text-lg font-medium text-[--text-normal] mb-2")}>
          From ScreenPipe
        </h3>
        <p className={tw("text-sm text-[--text-muted]")}>
          ScreenPipe unavailable. Start ScreenPipe (e.g. localhost:3030) to see
          meetings here.
        </p>
      </div>
    );
  ...
  const handleEnhanceNote = async (result: ScreenpipeResult) => {
    ...
    try {
      ...
      const recordingDate = result.content?.timestamp
        ? new Date(result.content.timestamp).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric",
          })
  ...
  const handleCreateNote = async (result: ScreenpipeResult) => {
    ...
    try {
      ...
      const appName = (c.app_name ?? "Meeting").replace(/[^\w\s-]/g, "").slice(0, 30) || "Meeting";
      ...
      if (!newFile || !(newFile instanceof TFile)) throw new Error("Failed to create file");
  ...
  return (
    <div className={tw("p-4 flex-1 overflow-y-auto")}>
      ...
      {results.length === 0 ? (
        <p className={tw("text-sm text-[--text-muted]")}>
          No meeting audio in the last {plugin.settings.screenpipeTimeRange ?? 2} hours.
        </p>
      ) : (
                ...
                <div className={tw("flex items-start justify-between mb-2 gap-2")}>
                  <div className={tw("flex-1 min-w-0 pr-2")}>
                    <div className={tw("flex items-start gap-2 mb-1 flex-wrap")}>
                      <span
                        className={tw("text-sm font-medium text-[--text-normal]")}
                      >
                        {c.app_name ?? "Meeting"}
                    ...
                    <div className={tw("text-xs text-[--text-muted] space-x-3")}>
                      <span>{c.timestamp ? formatDate(c.timestamp) : "—"}</span>
                    </div>
                    <p
                      className={tw("text-xs text-[--text-muted] mt-1 truncate")}
